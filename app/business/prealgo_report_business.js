var PreAlgoReportBusiness=function()
{

}
PreAlgoReportBusiness.prototype.showReport=function(reportName,reportDate,$reportContainer)
{
	var that=this;
	injector.process("widgetManager","@reportService","notifier","loadBar",function(widgetManager,service,notifier,loadBar){
		$reportContainer.empty();
		loadBar.show();
		service.getData("pre",reportName,reportDate).success(function(tableData){
			loadBar.hide();
...
PreAlgoReportBusiness.prototype.showReportTypes=function(reportName,reportDate,$reportTypeContainer)
{
	var that=this;
	injector.process("reportTypes","stringUtils","dateUtils",function(reportTypes,stringUtils,dateUtils){
		var reportTypes=reportTypes['pre-algo'];
		...
		reportTypes.forEach(function(type){
			...
			$reportTypeContainer.append("<li class='" + activeClass + "'><a href='#!/pre-algo-reports/" + typeObj.type + "/" + reportDate + "/'>" + typeObj.name + "</a></li>");
		})
	});

}
exports.PreAlgoReportBusiness=PreAlgoReportBusiness;